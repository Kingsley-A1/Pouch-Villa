...
**Product media manager, added 2026-09-03.** Recorded in
...
- **Images can be replaced in place**, on both screens. On the edit screen that
  ...
  delete first. Replaced renditions are deleted after the commit, and skipped
  when the content hash is unchanged: keys are content-hashed, so an identical
  re-upload writes the same objects.
- **Alt text is a real field.** Both screens stored the _filename_, which a
  screen reader reads out as "IMG 4021 dot jpeg". Not part of the reported
  fault, but created by the line that had to change, and §2 puts WCAG 2.2 AA at
  the floor.
- **One shared upload helper.** Both screens had their own copy of the
  begin → PUT → finalise exchange and their own idea of what a failure meant.

> **The upload failure itself was fixed in the catalogue-forms release above.**
> `presignUpload` signed the size _cap_ as `ContentLength`, which made the URL a
> promise the browser could not keep — a signature mismatch on every upload, on
> the admin path and the customer payment-receipt path alike. The presign test
...
| **Production is not indexable**        | 🟠 The live site cannot appear in search at all      | Found 2026-09-03 via PageSpeed Insights (SEO 69/100). `NEXT_PUBLIC_SITE_INDEXABLE=false` on the production deployment means `robots.txt` disallows everything and every page carries `noindex, nofollow` — correct for staging, wrong for `www.pouchvilla.com.ng`. Set it `true` on the production environment only. Not a code change. |
