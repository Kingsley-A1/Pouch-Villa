...
export function BrandMark({ inverse = false, compact = false }: { inverse?: boolean; compact?: boolean }) {
  return (
    <span className={cn("inline-flex items-center gap-2.5", inverse ? "text-white" : "text-[#e30613]")} aria-label="Pouch Hub">
      <span
        className={cn(
      ...
      <span className={cn("font-serif font-black uppercase leading-[.8] tracking-[-.055em]", compact ? "text-xl" : "text-2xl")}>
        Pouch <small className="block text-[.58em] tracking-[.04em]">Hub</small>
      </span>
    </span>
  );
}
