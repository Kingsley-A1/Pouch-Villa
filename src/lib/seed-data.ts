...
export function seedDatabase(db: DatabaseSync, adminEmail: string, adminPassword: string) {
  ...
  try {
    ...
    productNames.forEach((name, index) => {
      ...
      const variants = [
        { name: colour[0], color: colour[1], sku: `PV-${String(index + 1).padStart(3, "0")}-A`, availability: availability[index % availability.length] },
        { name: "Black", color: "#171717", sku: `PV-${String(index + 1).padStart(3, "0")}-B`, availability: "available" },
        { name: "Clear", color: "#e8e6e2", sku: `PV-${String(index + 1).padStart(3, "0")}-C`, availability: index % 4 === 0 ? "limited" : "available" },
      ];
      ...
      const compatible = db.prepare("SELECT id FROM devices WHERE brand_id = ? ORDER BY id LIMIT 4").all(selectedBrand.id) as Array<{ id: number }>;
      compatible.forEach((device) => linkDevice.run(productId, device.id));
    ...
    [
      ["PV-R-260701", "Demo Customer A", "Demonstration contact", "iPhone 14 Pro Max", "Blush", "+1 day", "Confirm exact model at pickup", "new", "-1 hour"],
      ["PV-R-260702", "Demo Customer B", "Demonstration contact", "Galaxy S24 Ultra", "Black", "+2 days", "Requested rugged protection", "contacted", "-5 hours"],
      ["PV-R-260703", "Demo Customer C", "Demonstration contact", "Camon 40 Pro", "Clear", "+1 day", "Pickup timing awaiting confirmation", "confirmed", "-1 day"],
      ["PV-R-260704", "Demo Customer D", "Demonstration contact", "iPhone 15 Pro Max", "Pouch Red", "+3 days", "Gift request", "ready", "-2 days"],
    ...
    enquiryInsert.run("PV-E-260701", "Demo Enquirer A", "Demonstration contact", firstProducts[1].id, "Is this available for Pixel 9 Pro?", "new", "-3 hours");
    enquiryInsert.run("PV-E-260702", "Demo Enquirer B", "Demonstration contact", firstProducts[2].id, "Please prepare a WhatsApp product summary.", "responded", "-1 day");
    ...
    requestInsert.run("PV-C-260701", "Demo Request A", "Demonstration contact", "Tecno", "Phantom V Fold", "Clear case, enhanced protection", "new", "-4 hours");
    requestInsert.run("PV-C-260702", "Demo Request B", "Demonstration contact", "Xiaomi", "Poco X7 Pro", "Soft colour, slim profile", "reviewing", "-2 days");
