...
export default function HomePage() {
  ...
  return <>
    ...
    <section className="relative overflow-hidden bg-[#fcfaf8]">
      <div className="container-shell grid min-h-[780px] items-center gap-10 py-14 lg:grid-cols-[1.05fr_.95fr] lg:py-20">
        <div className="relative z-10">
          <p className="eyebrow">Phone cases, fitted with confidence</p>
          <h1 className="display-title mt-5 max-w-3xl">Protect Your Phone.<br /><span className="text-[#e30613]">Show Your Style.</span></h1>
          <p className="mt-6 max-w-xl text-lg leading-8 text-zinc-600">Find demonstration cases matched to your exact phone model, then save, reserve or prepare an enquiry.</p>
          <div className="mt-9 rounded-[1.6rem] border border-[#e8e3df] bg-white p-5 shadow-[0_24px_70px_rgba(38,24,22,.08)] sm:p-6"><FindMyPhone brands={brands} devices={devices} /></div>
        </div>
        <div className="relative min-h-[390px] overflow-hidden rounded-[2rem] bg-[#f6f3f1] lg:min-h-[680px]">
          <Image src="/images/pouch-villa-hero.png" alt="Original demonstration phone cases in Pouch Villa colours" fill priority sizes="(max-width: 1024px) 100vw, 48vw" className="object-cover object-[67%_center]" />
        </div>
      </div>
    </section>
    ...
    <section className="section-space bg-[#171717] text-white"><div className="container-shell"><div className="mb-8 flex items-end justify-between"><div><p className="eyebrow">Curated entry points</p><h2 className="section-title mt-3">Featured collections</h2></div><Link href="/collections" className="button-ghost border-white/20 bg-transparent text-white hover:bg-white/10">All collections</Link></div><div className="grid gap-px overflow-hidden rounded-3xl bg-white/10 sm:grid-cols-2 lg:grid-cols-3">{collections.slice(0, 6).map((collection) => <Link href={`/collections/${collection.slug}`} key={collection.id} className="group min-h-52 bg-[#171717] p-6 transition hover:bg-[#231f1e]"><Sparkle size={25} className="text-[#e30613]" /><h3 className="mt-12 text-2xl font-bold">{collection.name}</h3><p className="mt-2 text-sm leading-6 text-zinc-400">{collection.description}</p></Link>)}</div></div></section>
    ...
    <section className="section-space"><div className="container-shell grid overflow-hidden rounded-[2rem] border border-[#e8e3df] lg:grid-cols-2"><div className="relative min-h-[420px]"><Image src="/images/pouch-villa-storefront.webp" alt="Pouch Villa storefront in Calabar" fill sizes="(max-width: 1024px) 100vw, 50vw" className="object-cover" /></div><div className="p-7 sm:p-10 lg:p-14"><p className="eyebrow">Visit the physical store</p><h2 className="section-title mt-3">See the cases in person.</h2><div className="mt-8 grid gap-5"><div className="flex gap-3"><MapPin className="mt-1 shrink-0 text-[#e30613]" size={23} /><div><p className="font-bold">Store address</p><p className="mt-1 text-zinc-600">{address || "Exact address awaiting Pouch Villa confirmation."}</p></div></div><div className="flex gap-3"><Clock className="mt-1 shrink-0 text-[#e30613]" size={23} /><div><p className="font-bold">Opening hours</p><p className="mt-1 text-zinc-600">{hours || "Opening hours awaiting Pouch Villa confirmation."}</p></div></div></div><div className="mt-8 flex flex-wrap gap-3"><Link href="/visit-us" className="button-primary">Visit Us</Link><WhatsAppEnquiry number={whatsapp} message="Hello Pouch Villa, I would like to confirm your official store address and opening hours before visiting." label="Prepare store enquiry" /></div></div></div></section>

    <section className="section-space bg-[#f6f3f1]"><div className="container-shell grid gap-10 lg:grid-cols-[.75fr_1.25fr]"><div><p className="eyebrow">Quick answers</p><h2 className="section-title mt-3">Before you reserve</h2><Link href="/help" className="button-ghost mt-6">See all FAQs</Link></div><div className="divide-y divide-[#dcd5d0]">{[["How do I know a case fits?","Select and confirm your exact phone model before reserving or preparing an enquiry."],["Does the prototype show live stock?","No. Availability is demonstration data until Pouch Villa confirms an operating process."],["Can I pay online?","No. Online payment is intentionally outside this prototype."],["Will WhatsApp send automatically?","No real message is sent. Without an approved number, you receive a safe message preview."]].map(([question, answer]) => <details key={question} className="group py-5"><summary className="cursor-pointer list-none font-bold">{question}</summary><p className="mt-3 max-w-2xl text-sm leading-6 text-zinc-600">{answer}</p></details>)}</div></div></section>
  </>;
}
