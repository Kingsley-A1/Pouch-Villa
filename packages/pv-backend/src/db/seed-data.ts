...
/** reference, name, contact, model, variant, pickup, notes, status, age */
type ReservationFixture = [string, string, string, string, string, string, string, string, string];
...
const devices: Record<string, Array<[string, string, number]>> = {
  apple: [
    ["iPhone 17 Pro Max", "iphone-17-pro-max", 2025],
    ["iPhone 16 Pro Max", "iphone-16-pro-max", 2024],
  ...
  samsung: [
    ["Galaxy S25 Ultra", "galaxy-s25-ultra", 2025],
    ["Galaxy S24 Ultra", "galaxy-s24-ultra", 2024],
    ["Galaxy S24", "galaxy-s24", 2024],
    ["Galaxy A56", "galaxy-a56", 2025],
    ["Galaxy A36", "galaxy-a36", 2025],
  ],
  google: [
    ["Pixel 9 Pro", "pixel-9-pro", 2024],
    ["Pixel 9", "pixel-9", 2024],
    ["Pixel 8 Pro", "pixel-8-pro", 2023],
  ],
  xiaomi: [
    ["Redmi Note 14 Pro", "redmi-note-14-pro", 2025],
    ["Redmi Note 13 Pro", "redmi-note-13-pro", 2024],
    ["Redmi 14C", "redmi-14c", 2024],
  ],
  tecno: [
    ["Camon 40 Pro", "camon-40-pro", 2025],
    ["Camon 30", "camon-30", 2024],
    ["Spark 30 Pro", "spark-30-pro", 2024],
    ["Pop 9", "pop-9", 2024],
  ],
  infinix: [
    ["Note 50 Pro", "note-50-pro", 2025],
    ["Hot 50 Pro+", "hot-50-pro-plus", 2024],
    ["Smart 9", "smart-9", 2024],
  ],
};
...
const collections = [
  ...
  [
    "Rugged Protection",
    "rugged-protection",
    "Reinforced demonstration options for demanding days.",
  ],
...
export function seedDatabase(db: DatabaseSync, adminEmail: string, adminPassword: string) {
  ...
  try {
    ...
    const insertDevice = db.prepare(
      "INSERT INTO devices (brand_id, name, slug, released_year) VALUES (?, ?, ?, ?)",
    );
    brands.forEach(([name, slug], index) => {
      const result = insertBrand.run(name, slug, index);
      const brandId = Number(result.lastInsertRowid);
      must(devices[slug], `devices for brand ${slug}`).forEach(([deviceName, deviceSlug, year]) =>
        insertDevice.run(brandId, deviceName, deviceSlug, year),
      );
    });
    ...
    collections.forEach(([name, slug, description], index) =>
      insertCollection.run(name, slug, description, index),
    );
    ...
    const linkDevice = db.prepare(
      "INSERT INTO product_devices (product_id, device_id) VALUES (?, ?)",
    );
    const linkCollection = db.prepare(
      "INSERT INTO product_collections (product_id, collection_id) VALUES (?, ?)",
    );
    const allBrandRows = db
      .prepare("SELECT id, slug FROM brands ORDER BY sort_order")
      .all() as Array<{ id: number; slug: string }>;

    productNames.forEach((name, index) => {
      const visual = index % images.length;
      const colour = at(colours, visual);
      const productSlug = name.toLowerCase().replace(/\s+/g, "-");
      const variants = [
        {
          name: colour[0],
          color: colour[1],
          sku: `PH-${String(index + 1).padStart(3, "0")}-A`,
          availability: at(availability, index % availability.length),
        },
        {
          name: "Black",
          color: "#171717",
          sku: `PH-${String(index + 1).padStart(3, "0")}-B`,
          availability: "available",
        },
        {
          name: "Clear",
          color: "#e8e6e2",
          sku: `PH-${String(index + 1).padStart(3, "0")}-C`,
          availability: index % 4 === 0 ? "limited" : "available",
        },
      ];
      ...
      const result = insertProduct.run(
        ...
        12500 + (index % 8) * 1750,
        at(availability, index % availability.length),
        at(styles, visual),
        at(materials, visual),
        at(protections, visual),
        index % 3 === 0 ? 1 : 0,
        index < 10 ? 1 : 0,
        index % 4 === 0 ? 1 : 0,
        at(images, visual),
        ...
        offset,
        offset,
        index % 7 === 0 ? "-24 days" : offset,
      );
      const productId = Number(result.lastInsertRowid);
      const selectedBrand = at(allBrandRows, index % allBrandRows.length);
      ...
      const brandDevices = db
        .prepare("SELECT id FROM devices WHERE brand_id = ? ORDER BY id")
        .all(selectedBrand.id) as Array<{ id: number }>;
      ...
      for (let step = 0; step < Math.min(4, brandDevices.length); step += 1) {
        compatible.add(at(brandDevices, (start + step) % brandDevices.length).id);
      }
    ...
    orphanDevices.forEach((device) => {
      const match = (leastLinkedForBrand.get(device.brand_id) ?? anyProduct.get()) as
        { id: number } | undefined;
      if (match) linkDevice.run(match.id, device.id);
    });

    const firstProducts = db
      .prepare("SELECT id, name FROM products ORDER BY id LIMIT 5")
    ...
    (
      [
        ...
        [
          ...
          "Clear",
          "+1 day",
          "Pickup timing awaiting confirmation",
          "confirmed",
          "-1 day",
        ],
        [
      ...
      ] as ReservationFixture[]
    ).forEach(([reference, name, contact, model, variant, pickup, notes, status, age], index) =>
      reservationInsert.run(
        reference,
        name,
        contact,
        model,
        at(firstProducts, index).id,
        variant,
        pickup,
        notes,
        status,
    ...
    const enquiryInsert = db.prepare(
      "INSERT INTO enquiries (reference, customer_name, contact, product_id, message, status, created_at) VALUES (?, ?, ?, ?, ?, ?, datetime('now', ?))",
    );
    enquiryInsert.run(
      "PH-E-260701",
      "Demo Enquirer A",
      "Demonstration contact",
      at(firstProducts, 1).id,
      "Is this available for Pixel 9 Pro?",
      "new",
      "-3 hours",
    );
    enquiryInsert.run(
      ...
      "Demo Enquirer B",
      "Demonstration contact",
      at(firstProducts, 2).id,
      "Please prepare a WhatsApp product summary.",
      "responded",
      "-1 day",
    );

    const requestInsert = db.prepare(
      "INSERT INTO case_requests (reference, customer_name, contact, brand, model, preferences, status, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, datetime('now', ?))",
    );
    requestInsert.run(
      "PH-C-260701",
      "Demo Request A",
      ...
      "Tecno",
      "Phantom V Fold",
      "Clear case, enhanced protection",
      "new",
      "-4 hours",
    );
    requestInsert.run(
      ...
      "Xiaomi",
      "Poco X7 Pro",
      "Soft colour, slim profile",
      "reviewing",
      "-2 days",
    );

    const eventInsert = db.prepare(
      "INSERT INTO analytics_events (event_type, entity, value, created_at) VALUES (?, ?, ?, datetime('now', ?))",
    );
    [
      "iPhone 14 Pro Max",
      "Galaxy S24 Ultra",
      ...
      for (let count = 0; count < 7 - index; count += 1)
        eventInsert.run("device_selected", "device", model, `-${count + index} hours`);
    });
    ["iphone 12 mini", "itel a70", "poco c75"].forEach((term, index) =>
      eventInsert.run("search_no_results", "search", term, `-${index + 1} days`),
    );
    ["new-arrivals", "rugged-protection", "soft-colours", "magsafe"].forEach((slug, index) => {
      for (let count = 0; count < 5 - index; count += 1)
        eventInsert.run("collection_view", "collection", slug, `-${count + 1} hours`);
    });
