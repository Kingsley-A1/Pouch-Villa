...
function openDatabase() {
  ...
  try {
    ...
    console.error(
      `Database path ${databasePath} is not writable; falling back to in-memory.`,
      error,
    );
...
export function getDevices(brandSlug?: string) {
  ...
  return all<Device>(
    `SELECT d.*, b.name AS brand_name, b.slug AS brand_slug FROM devices d JOIN brands b ON b.id = d.brand_id ${clause} ORDER BY b.sort_order, d.released_year DESC, d.name`,
    ...params,
  );
}
...
export type ProductFilters = {
  ...
  magsafe?: string | undefined;
  min?: string | undefined;
  max?: string | undefined;
  collection?: string | undefined;
  newOnly?: string | undefined;
...
export function getProducts(filters: ProductFilters = {}) {
  const clauses = [
    filters.includeAdmin ? "1=1" : "p.status = 'published' AND p.availability != 'hidden'",
  ];
  const params: SQLInputValue[] = [];
  const add = (condition: string, value: SQLInputValue) => {
    clauses.push(condition);
    params.push(value);
  };
  if (filters.q) {
    add("(p.name LIKE ? OR p.description LIKE ?)", `%${filters.q}%`);
  ...
  if (filters.min && Number.isFinite(Number(filters.min)))
    add("p.demo_price >= ?", Number(filters.min));
  if (filters.max && Number.isFinite(Number(filters.max)))
    add("p.demo_price <= ?", Number(filters.max));
  ...
  return all<Product>(
    ...
    ORDER BY p.is_new DESC, p.is_bestseller DESC, p.created_at DESC, p.name`,
    ...params,
  );
}
...
export function getProductBySlug(slug: string, includeAdmin = false) {
  const product = one<Product>(
    `SELECT * FROM products WHERE slug = ? ${includeAdmin ? "" : "AND status = 'published' AND availability != 'hidden'"}`,
    slug,
  );
  if (!product) return undefined;
  product.devices = all<Device>(
    "SELECT d.*, b.name AS brand_name, b.slug AS brand_slug FROM devices d JOIN brands b ON b.id = d.brand_id JOIN product_devices pd ON pd.device_id = d.id WHERE pd.product_id = ? ORDER BY b.sort_order, d.name",
    product.id,
  );
  product.collections = all<Collection>(
    "SELECT c.* FROM collections c JOIN product_collections pc ON pc.collection_id = c.id WHERE pc.product_id = ? ORDER BY c.sort_order",
    product.id,
  );
  return product;
}
...
export function getReservations() {
  return all<Reservation>(
    "SELECT r.*, p.name AS product_name FROM reservations r LEFT JOIN products p ON p.id = r.product_id ORDER BY r.created_at DESC",
  );
}
