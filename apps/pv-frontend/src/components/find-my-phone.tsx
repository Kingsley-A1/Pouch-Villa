...
import type { Brand, Device } from "@pv/backend/domain/types";

export type RememberedPhone = {
  brand: string;
  brandName: string;
  model: string;
  modelName: string;
...
export function FindMyPhone({
  ...
  const models = useMemo(
    () => devices.filter((item) => item.brand_slug === brand),
    [brand, devices],
  );
  ...
  return (
    <form onSubmit={submit} className={compact ? "grid gap-4" : "grid max-w-xl gap-4"}>
      ...
      <button type="submit" className="button-primary w-full sm:w-auto" disabled={!brand || !model}>
        <DeviceMobile size={21} weight="bold" /> Show compatible cases{" "}
        <ArrowRight size={19} weight="bold" />
      </button>
      {brand && model ? (
        <p className="flex items-center gap-2 text-sm font-semibold text-emerald-700">
          <CheckCircle size={18} weight="fill" /> Device confirmed before browsing.
        </p>
      ) : null}
    </form>
  );
