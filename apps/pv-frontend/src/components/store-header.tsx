...
const links = [
  ["Shop", "/shop"],
  ["Find My Case", "/find-my-case"],
  ["Collections", "/collections"],
  ["Request a Case", "/request-case"],
  ["Visit Us", "/visit-us"],
  ["Help", "/help"],
] as const;
      ...
      <header className="sticky top-0 z-40 border-b border-[#e8e3df] bg-white/95 backdrop-blur-sm">
        <div className="container-shell flex h-[76px] items-center justify-between gap-5">
          ...
          <nav className="hidden items-center gap-6 lg:flex" aria-label="Main navigation">
            {links.slice(0, 5).map(([label, href]) => (
              <Link
                key={href}
                href={href}
                className="text-sm font-semibold text-zinc-700 transition-colors hover:text-[#e30613]"
              >
                {label}
              </Link>
            ))}
          ...
          <div className="flex items-center gap-1.5">
            ...
            <Link
              href="/visit-us"
              className="hidden h-11 items-center gap-2 rounded-xl border border-[#e8e3df] px-3 text-sm font-bold hover:border-[#e30613] sm:flex"
            >
              <MapPin size={19} /> Visit
            </Link>
